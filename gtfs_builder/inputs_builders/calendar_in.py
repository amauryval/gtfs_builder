...
class Calendar(OpenGtfs):
    ...
    def _format_date_column(self) -> None:
        self._input_data["start_date"] = [
            datetime.datetime.strptime(row, '%Y%m%d') for row in self._input_data["start_date"]]
        self._input_data["end_date"] = [
            datetime.datetime.strptime(row, '%Y%m%d') for row in self._input_data["end_date"]]
