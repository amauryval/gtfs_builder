from starlite import Router, Provide, Controller, get, State
...
class FromFileController(Controller):
    ...
    def moving_nodes_by_date(self,
        ...
        return GtfsMain(state.dict()["geodata"][area]).nodes_by_date_from_parquet(
            current_date,
    ...
    def range_dates(self, area: str, state: State) -> dict:
        return GtfsMain(state.dict()["geodata"][area]).context_data_from_parquet()

    @get("/{area:str}/route_types")
    def transport_types(self, area: str, state: State) -> list[str]:
        return GtfsMain(state.dict()["geodata"][area]).transport_types_from_parquet()
